...
The mutation request must contain only the transaction ID and the one intended review-state field. The verified upstream 1.5.2 helper currently serializes unrelated `category: null` and `name: null`; this path must not be used until upstream tests or a controlled contract probe proves those fields cannot alter data, or the upstream adapter/client provides a narrow request that omits them. Verification also checks that category and merchant identity were not changed.
...
- **No unrelated nullable inputs.** A review-only mutation cannot silently transmit category or merchant updates.
...
- [ ] Before implementation uses the released upstream helper, evidence establishes that its nullable unrelated inputs are harmless, or a narrow supported adapter/client path omits them.
